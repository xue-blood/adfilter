...
tdi_send_datagram(PIRP irp, PIO_STACK_LOCATION irps, struct completion *completion)
{
	...
	PUCHAR data = (PUCHAR)MmGetSystemAddressForMdl(irp->MdlAddress);
	if (!data) data = irp->AssociatedIrp.SystemBuffer;
	if (!data) data = irp->UserBuffer;
	if (!data) KdPrint(("[tdifw] get data buffer failed.\n"));
	result = tdifw_udp_send(data, param->SendLength);
